...
- **Apps VPS** → **AI server:11434** (Ollama) over Tailscale IPs.
- **OpenClaw VPS** → **AI server:11434** (for embeddings / optional local LLM provider).
...
- **AI server** Tailscale IP — used by Open WebUI on the **apps VPS** as `OLLAMA_BASE_URL` ([../vps-apps/02-open-webui/README.md](../vps-apps/02-open-webui/README.md)) and by OpenClaw memory plugins on the **OpenClaw VPS** as the Ollama `baseUrl` ([../vps-openclaw/02-memory/README.md](../vps-openclaw/02-memory/README.md)).
