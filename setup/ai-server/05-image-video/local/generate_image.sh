#!/usr/bin/env bash
# One-shot: health check + ComfyUI txt2img. Run on OpenClaw VPS.
# Usage: generate_image.sh "prompt text" [output.png]
set -euo pipefail
...
mkdir -p "$(dirname "$OUT")"
python3 "$DIR/comfyui_health.py" || { echo '{"ok":false,"error":"comfyui health failed"}'; exit 1; }
